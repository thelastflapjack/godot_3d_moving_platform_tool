tool
class_name PhysicsPlatformFollowPath
extends Spatial
# Docstring
...
### Onready variables ###
onready var _platform: KinematicBody = get_node("Platform")
onready var _tween: Tween = get_node("Tween")
onready var _closed_loop_line: ImmediateGeometry = get_node("ClosedLoopLine")
onready var _point_res: PackedScene = preload("res://moving_platform_system/point.tscn")
...
func _find_current_points() -> void:
	for child in get_children():
		if child is PhysicsPlatformFollowPathPoint:
			var point: PhysicsPlatformFollowPathPoint = child
			_points.append(point)
			
			point.set_mesh($Platform/MeshInstance.get_mesh())
			...
			if _points.size() == 1:
				...
				var previous_point: PhysicsPlatformFollowPathPoint = _points[previous_point_index]
...
func _add_point() -> void:
	if Engine.editor_hint && is_inside_tree():
		var point: PhysicsPlatformFollowPathPoint = _point_res.instance()
		add_child(point)
...
func _get_point(index: int) -> PhysicsPlatformFollowPathPoint:
...
func _tween_to_next_point() -> void:
	var current_point: PhysicsPlatformFollowPathPoint = _get_point(_current_point_index)
	var next_point: PhysicsPlatformFollowPathPoint = _get_point(_next_point_index)
